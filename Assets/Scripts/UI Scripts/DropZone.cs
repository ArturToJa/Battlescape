...
public class DropZone : MonoBehaviour//, IDropHandler
{
    ...
    public void CommandInstantiateUnit(int UnitID, int PlayerID)
    {
        ...
        {
            GetComponent<PhotonView>().RPC("RPCInstantiateUnit", PhotonTargets.All, UnitID, PlayerID, index);
    ...
    public void SendCommandToSetUnitPosition(Unit unit, MultiTile targetPosition)
    {
        ...
        if (Global.instance.matchType == MatchTypes.Online)
        {
            photonView.RPC("RPCSetUnitPosition", PhotonTargets.All, unit.index, endPosX, endPosZ);
        }
        else
        {
