...
namespace BattlescapeLogic
{
    public class Cursor : MonoBehaviour
    {
        ...
        void SetCursorTo(Texture2D basicCursor, Texture2D clickingCursor)
        {
            ...
            {
                UnityEngine.Cursor.SetCursor(clickingCursor, GetHotspotType(clickingCursor), CursorMode.Auto);
                return;
            }
            else
            {
                UnityEngine.Cursor.SetCursor(basicCursor, GetHotspotType(basicCursor), CursorMode.Auto);
                return;
            }
        }
