...
public class Ability_Neutral_SwapWeapons : Ability_Basic
{
    ...
    IEnumerator Swap(Weapon swapIn, Weapon swapOut)
    {
        ...
        myUnit.isRanged = swapIn.IsRanged;
        myUnit.GetComponent<ShootingScript>().enabled = swapIn.IsRanged;
        GetComponentInChildren<AnimController>().SetRanged(swapIn.IsRanged);
